  ...
  <div class="card-container">
    ...
    <el-card class="order-card" shadow="never">
      <div v-for="order in tableData" :key="order.id" class="order-item">
        ...
        <div style="background-color: rgb(245,246,247);margin-bottom: 20px;">
          <!-- 收件人信息 -->
          <div v-if="order.concatPerson" style="padding: 10px;box-sizing: border-box;margin-bottom: 6px;">
            <div style="font-size: 18px;font-weight: 600;margin-bottom: 6px;">收件人地址信息</div>
            <div style="margin-bottom: 4px;">
              <el-tag type="warning">收件人：{{ order.concatPerson }}</el-tag>
              <el-tag type="warning">收件地址：{{ order.getAdr }}</el-tag>
              <el-tag type="warning">收件人联系电话：{{ order.concatPhone }}</el-tag>
            </div>
          </div>
          <div v-if="order.adrName" style="padding: 10px;box-sizing: border-box;margin-bottom: 1px;">
            <div style="font-size: 18px;font-weight: 600;margin-bottom: 6px;">收件人地址信息</div>
            <div style="margin-bottom: 4px;">
              <el-tag type="warning">发件人：{{ order.adrName }}</el-tag>
              <el-tag type="warning">发件地址：{{ order.adr }}</el-tag>
              <el-tag type="warning">发件人联系电话：{{ order.adrPhone }}</el-tag>
            </div>
          </div>
        </div>
        ...
        <div class="order-content">
          ...
          <div class="order-status">
            <el-tag :type="order.tradeStatus ? 'success' : 'warning'" size="small">
              订单状态：{{ order.tradeStatus === 1 ? '待支付' : (order.tradeStatus === 2 ?
              '已支付' : '已取消') }}
            </el-tag>
            <el-tag :type="order.refundStatus ? 'primary' : 'danger'" size="small" style="margin-top: 5px;">
              退款状态：{{ order.refundStatus === 1 ? '无退款' : (order.refundStatus === 2 ?
              '买家申请退款，请尽快审核' : (order.refundStatus === 3 ? '拒绝退款' : '已退款')) }}
            </el-tag>
          </div>
