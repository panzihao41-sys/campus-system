  ...
  <div class="order-container">
    ...
    <transition-group v-else name="order-list" tag="div" class="order-list">
      <div v-for="(orderInfo, index) in ordersList" :key="orderInfo.id" class="order-card"
        ...
        <div v-if="orderInfo.concatPerson" style="padding: 10px;box-sizing: border-box;margin-bottom: 1px;">
          <div style="font-size: 18px;font-weight: 600;margin-bottom: 6px;">收件人地址信息</div>
          <div style="margin-bottom: 4px;">
            <el-tag type="warning">收件人：{{ orderInfo.concatPerson }}</el-tag>
            <el-tag type="warning">收件地址：{{ orderInfo.getAdr }}</el-tag>
            <el-tag type="warning">收件人联系电话：{{ orderInfo.concatPhone }}</el-tag>
          </div>
        </div>
        <div v-if="orderInfo.adrName" style="padding: 10px;box-sizing: border-box;margin-bottom: 1px;">
          <div style="font-size: 18px;font-weight: 600;margin-bottom: 6px;">收件人地址信息</div>
          <div style="margin-bottom: 4px;">
            <el-tag type="warning">发件人：{{ orderInfo.adrName }}</el-tag>
            <el-tag type="warning">发件地址：{{ orderInfo.adr }}</el-tag>
            <el-tag type="warning">发件人联系电话：{{ orderInfo.adrPhone }}</el-tag>
          </div>
        </div>
        ...
        <div class="order-footer">
          ...
          <div class="action-buttons">
            ...
            <el-button type="primary"
              v-if="(orderInfo.refundStatus === 1 || orderInfo.refundStatus === 3) && orderInfo.tradeStatus === 2"
              @click="toRefund(orderInfo)">申请退款</el-button>
